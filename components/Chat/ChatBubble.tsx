...
const ChatBubble: React.FC<Message> = ({ from, message, time }) => {
  ...
  return (
    <li
      className={`${from === "client" ? "self-end" : "self-start"} w-2/3 border rounded p-1 m-2 divide-y divide-slate-300 shadow-md dark:border-slate-600/50 dark:divide-slate-600/50`}>
      <div className={"flex justify-between"}>
        <h3 className={"mx-3"}>{from === "client" ? data?.user?.name : "Awatansa [ bot 🟢 ]"}</h3><span
